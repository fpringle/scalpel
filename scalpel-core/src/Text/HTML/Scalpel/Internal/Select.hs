...
tagsToVector tags =
  ...
  where
    ...
    go [] state =
      ...
      where
        maybeName t
          | TagSoup.isTagOpen t = Just $ getTagName t
...
vectorToTree tags = fixup $ forestWithin 0 (Vector.length tags)
  where
    ...
    forestWithin !lo !hi
      ...
      | otherwise =
          Tree.Node (Span lo closeIndex) subForest
    ...
    fixup (Tree.Node (Span lo hi) subForest : siblings) =
      ...
      where
        ...
        malformed ::
          TagForest -> -- Forest to prepend bad trees on.
...
selectNodes (n : ns) cur@(tags, f : fs, ctx) root acc
  | MatchOk == matchResult =
      selectNodes
        ns
        (tags, Tree.subForest f ++ siblings, ctx)
...
nodeMatches (SelectText, settings) info cur root =
  checkSettings settings cur root
    `andMatch` boolMatch (TagSoup.isTagText $ infoTag info)
